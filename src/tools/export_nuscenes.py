...
class KittiConverter:
    def __init__(self,
                 nusc_dataroot: str = '/root/autodl-tmp/RTM3D/kitti_format/data/nuscenes/',
                 nusc_kitti_dir: str = '/root/autodl-tmp/RTM3D/kitti_format/data/nuscenes_kitti/',
                 cam_name: str = 'CAM_FRONT',
                 lidar_name: str = 'LIDAR_TOP',
                 image_count: int = -1,
                 nusc_version: str = 'v1.0-trainval',
                 parallel_n_jobs: int = 64,
                 split: str = 'train'):
        """
        ...
        self.nusc = NuScenes(version=nusc_version, dataroot=nusc_dataroot)
        ...
        self.label_folder = os.path.join(self.nusc_kitti_dir, self.split, 'label_2')
        self.calib_folder = os.path.join(self.nusc_kitti_dir, self.split, 'calib')
        self.image_folder = os.path.join(self.nusc_kitti_dir, self.split, 'image_2')
        # lidar_folder = os.path.join(self.nusc_kitti_dir, self.split, 'velodyne')
        for folder in [self.label_folder, self.calib_folder, self.image_folder]:
        ...
        with parallel_backend("threading", n_jobs=self.parallel_n_jobs):
            Parallel()(delayed(self.nuscenes_gt_to_kitti)(sample_token) for sample_token in tqdm(sample_tokens))

    def nuscenes_gt_to_kitti(self, sample_token: str) -> None:
        ...
        dst_im_path = os.path.join(self.image_folder, sample_token + '.png')
        ...
        calib_path = os.path.join(self.calib_folder, sample_token + '.txt')
        ...
        # Write label file.
        label_path = os.path.join(self.label_folder, sample_token + '.txt')
        ...
        with open(label_path, "w") as label_file:
            for sample_annotation_token in sample_annotation_tokens:
                ...
                bbox_2d = KittiDB.project_kitti_box_to_image(box_cam_kitti, p_left_kitti, imsize=self.imsize)
                if bbox_2d is None:
                    continue
